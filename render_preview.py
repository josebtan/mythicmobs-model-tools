...
def render(elev, azim, title, outfile):
    ...
    lim = max(abs(v) for v in all_pts) + 1
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
    ax.set_zlim(0, 20)
    ax.set_box_aspect([1, 1, 20 / (2 * lim)])
...
render(elev=15, azim=-60, title="Isometric", outfile="preview_iso.png")
render(elev=0, azim=-90, title="Front", outfile="preview_front.png")
render(elev=0, azim=0, title="Side", outfile="preview_side.png")
